  ...
  return (
    <div className="w-[min(1300px,calc(100%-40px))] mx-auto mt-10">
        ...
        <div className="flex items-center justify-between gap-[30px] mb-10">
            ...
            <h1 className='text-3xl'>New Releases</h1>
        ...
        {data.map((e)=>{
          return <SwiperSlide key={e.id} >
            <div onClick={()=> navigate(e.release_date ? `/movie-detail/${e.id}` : `/show-detail/${e.id}`)} className="border border-[#262626] rounded-[10px] bg-[#1a1a1a] p-5 min-h-[280px] flex flex-col justify-between">
              ...
              <div className="flex items-center justify-center text-white font-semibold">
                ...
                <div className='flex gap-1 border rounded-full py-1 px-2'>
                     <p className='text-sm font-light'>Released at {e.release_date || e.first_air_date}</p>
                </div>
